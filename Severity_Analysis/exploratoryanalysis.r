...
train<- Data %>% select(-X) %>% 
 mutate(across(c(claimAm), as.numeric)) %>%
 mutate(across(c(agecar, sexph, power, split, fuel, use, fleet, sportc, cover, ageph), as.factor))
...
for(i in colnames(train)){
  plot[[i]] <-   ggplot(data = train, mapping = aes_string(x = i, y = "claimAm")) +
...
grid.arrange(plot[[1]], plot[[9]], plot[[4]], 
            plot[[5]], plot[[6]], plot[[7]], 
            plot[[8]], plot[[10]], plot[[11]], plot[[12]], ncol= 3)


#map of balgium 
post_expo <- train %>% group_by(postal) %>% summarize(num = n(), tot_freq = sum(freq))
post_expo %>% slice(1:5)
